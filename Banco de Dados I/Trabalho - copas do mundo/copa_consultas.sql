...
WHERE id_jogador NOT IN (SELECT id_jogador FROM joga_evento);

select * from joga_evento;
delete from joga_evento where id_jogador=3;
...
SELECT participante.id_participante, participante.nome_participante,COUNT(premiacao.id_participante) AS qnt_premios
	...
	GROUP BY participante.id_participante;
...
INSERT INTO joga_evento(id_jogador, ano_copa, id_selecao) VALUES(21,2002,1); 
...
select retirar_jogador_evento(21); --valores pegos da criação de teste da consulta anterior
...
BEGIN
	...
	IF (TG_OP = 'DELETE') THEN
		SELECT count(id_ingresso) INTO qtd_ingressos FROM vende WHERE id_jogo = OLD.id_jogo;
		UPDATE jogo SET ingressos_vendidos = qtd_ingressos WHERE id_jogo = OLD.id_jogo;
	ELSIF (TG_OP = 'INSERT') THEN
		SELECT count(id_ingresso) INTO qtd_ingressos FROM vende WHERE id_jogo = NEW.id_jogo;
		UPDATE jogo SET ingressos_vendidos = qtd_ingressos WHERE id_jogo = NEW.id_jogo;
	END IF;
  RETURN NULL;
END;
$$ LANGUAGE plpgsql;

--assumimos que a alteração de ingresso em jogos é como em shows e cinemas: cancela-se primeiro e compra um novo
--não existindo, assim, a possibilidade de apenas fazer um 'update' para trocas de ingressos em jogos diferentes.
--O update pode acontecer em um mesmo jogo para uma outra seção pagando ou devolvendo a diferença de preço. 
--Nesse caso, a quantidade de ingressos permanece a mesma.
CREATE TRIGGER atualizar_ingressos_jogo_trigger AFTER 
INSERT OR DELETE
ON vende FOR EACH ROW 
...
select*from vende;
select *from jogo;
select cancelar_compra_ingresso(16);
select cancelar_compra_ingresso(19);
